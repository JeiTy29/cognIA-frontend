...
function mapCreateProblemReportError(error: unknown) {
    ...
    if (normalized.includes('invalid_user') || status === 401) return 'Tu sesi?n no es v?lida. Inicia sesi?n nuevamente.';
    if (normalized.includes('inactive_account') || status === 403) return 'Tu cuenta est? inactiva.';
    if (normalized.includes('attachment_missing')) return 'No se pudo procesar el archivo adjunto.';
    if (normalized.includes('attachment_filename_missing')) return 'El archivo adjunto debe conservar un nombre v?lido.';
    if (normalized.includes('attachment_mime_not_allowed')) return 'Solo se permiten archivos PNG, JPG o WEBP.';
    if (normalized.includes('attachment_empty')) return 'El archivo adjunto est? vac?o.';
    if (normalized.includes('attachment_too_large')) return 'El archivo adjunto supera el m?ximo de 5 MB.';
    ...
    if (status >= 500) return 'No fue posible consultar reportes en este momento. Intenta nuevamente m?s tarde.';
...
type SelectedAttachment = Readonly<{
    file: File | null;
...
function resolveSelectedAttachment(file: File | null): SelectedAttachment {
    if (!file) {
        return { file: null, previewUrl: null, error: null, clearInput: false };
    }
...
async function syncProblemReportsAfterCreate(
    showMyReports: boolean,
    reloadReports: () => Promise<void>,
    setShowMyReports: (value: boolean) => void
) {
    if (showMyReports) {
        await reloadReports();
        return;
...
const faqsPadre = [
    { id: 'padre-1', question: '?Qu? significa la alerta que muestra el sistema?', answer: 'La alerta indica un posible trastorno seg?n el cuestionario. No es un diagn?stico cl?nico.' },
    { id: 'padre-2', question: '?C?mo diligencio el cuestionario correctamente?', answer: 'Responde con honestidad y completa todas las preguntas para obtener un resultado confiable.' },
    { id: 'padre-3', question: '?D?nde veo el historial de mis cuestionarios?', answer: 'En la secci?n Historial puedes consultar los cuestionarios realizados por tu cuenta.' },
    { id: 'padre-4', question: '?Qu? datos se guardan sobre mi hijo?', answer: 'Solo se registran respuestas de comportamiento sin datos personales identificables.' }
];

const faqsPsicologo = [
    { id: 'psicologo-1', question: '?C?mo interpreto los resultados detallados?', answer: 'Los resultados muestran la alerta y el razonamiento del modelo para apoyar tu an?lisis profesional.' },
    { id: 'psicologo-2', question: '?Puedo ver m?ltiples evaluaciones de un mismo caso?', answer: 'S?, si el tutor otorg? permisos, puedes acceder al historial completo de cuestionarios.' },
    { id: 'psicologo-3', question: '?Qu? hacer si necesito reportes adicionales?', answer: 'Puedes usar la secci?n de reporte de problemas o contactarnos para solicitar soporte.' },
    { id: 'psicologo-4', question: '?C?mo se muestran las sugerencias del sistema?', answer: 'Las sugerencias se presentan como apoyo y no reemplazan la evaluaci?n cl?nica.' }
];
...
export default function AyudaBase({ role }: AyudaBaseProps) {
    ...
    const roleLabel = resolvedRole === 'psicologo' ? 'Psic?logo' : 'Padre/Tutor';
    ...
    const handleReportSubmit = async (event: FormEvent) => {
        ...
        const validationError = validateProblemReportSubmission(issueType, trimmedDescription);
        if (validationError) {
        ...
        try {
            ...
            resetReportForm();
            setShowReportForm(false);
            setReportSuccess(resolveProblemReportSuccessMessage(response.report.report_code));
            await syncProblemReportsAfterCreate(showMyReports, reloadReports, setShowMyReports);
        } catch (submitError) {
    ...
    const handleFileChange = (file: File | null) => {
        if (previewUrl) URL.revokeObjectURL(previewUrl);
        const selectedAttachment = resolveSelectedAttachment(file);
        setSelectedFile(selectedAttachment.file);
        setPreviewUrl(selectedAttachment.previewUrl);
        setReportError(selectedAttachment.error);
        if (selectedAttachment.clearInput && fileInputRef.current) {
                ...
                <div className="ayuda-container">
                    ...
                    <section className="ayuda-section ayuda-report-section">
                        <button type="button" className="ayuda-toggle" onClick={handleReportToggle}>
                            ...
                            <span className={`ayuda-toggle-icon ${showReportForm ? 'open' : ''}`}>
                                <ChevronIcon />
                            </span>
                        </button>
                            ...
                            <form className="ayuda-report-form" onSubmit={handleReportSubmit}>
                                <div className="ayuda-input-group">
                                    ...
                                    <div className="ayuda-select" ref={selectRef}>
                                        ...
                                        >
                                            <span>{selectedIssueOption?.label ?? 'Selecciona una opci?n'}</span>
                    ...
                    <section className="ayuda-section ayuda-my-reports-section">
                        <button type="button" className="ayuda-toggle" onClick={handleReportsToggle}>
                            ...
                            <span className={`ayuda-toggle-icon ${showMyReports ? 'open' : ''}`}>
                                <ChevronIcon />
                            </span>
                        </button>
                            ...
                            <div className="ayuda-reports-controls">
                                <div className="ayuda-reports-filters">
                                    ...
                                    <label>
                                        <span>Tipo</span>
                                        <CustomSelect
                                            ariaLabel="Filtrar reportes por tipo"
                                ...
                                <>
                                        ...
                                        {items.map((item) => (
                                            <article key={item.id} className="ayuda-report-item">
                                                <div className="ayuda-report-item-head">
                                                    ...
                                                    <span className={`ayuda-report-badge ${getStatusTone(item.status)}`}>
                                                ...
                                                <div className="ayuda-report-item-actions">
                                                    <button type="button" className="ayuda-btn ghost" onClick={() => setSelectedReport(item)}>
                                                        Ver detalle
                                                    </button>
                                                </div>
                                            </article>
                                    ...
                                    <footer className="ayuda-report-pagination">
                                        ...
                                        <div className="ayuda-report-pagination-controls">
                                            ...
                                            <span className="ayuda-page-current">P?gina {currentPage}</span>
                                            <button
                                                type="button"
                                                className="ayuda-page-btn"
                    ...
                    <section className="ayuda-section ayuda-contact-grid">
                        <div className="ayuda-block">
                            ...
                            <SupportContact roleLabel={roleLabel} moduleLabel="Ayuda" />
                        </div>
                ...
                <Modal isOpen={selectedReport !== null} onClose={() => setSelectedReport(null)}>
                    <div className="ayuda-detail-modal">
                        ...
                        {selectedReport ? (
                            <>
                                <div className="ayuda-detail-grid">
                                    ...
                                    <div>
                                        <strong>Tipo</strong>
                                        <span>{getProblemReportIssueTypeLabel(selectedReport.issue_type)}</span>
                                    </div>
                                    <div>
                                        <strong>Estado</strong>
                                        <span>{getProblemReportStatusLabel(selectedReport.status)}</span>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <span>{selectedReport.attachment_count}</span>
                                    </div>
                                </div>
                        ...
                        <div className="ayuda-detail-actions">
                            <button type="button" className="ayuda-btn ghost" onClick={() => setSelectedReport(null)}>
                                Cerrar
                            </button>
                        </div>
                    </div>
