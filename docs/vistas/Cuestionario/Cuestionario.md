...
La introducción se presenta sin cards, centrada verticalmente y con layout en dos columnas.
...
- Todas las demás preguntas (anteriores o posteriores) quedan arriba en formato compacto:
...
- Al avanzar: la pregunta activa sube, reduce tamaño/opacidad y pasa a "respondida".
...
- Encabezado con título, subtítulo y disclaimer breve.
- La barra de progreso aparece **debajo de la pregunta activa**, con:
  - **Pregunta X / N**
  - Barra de progreso (transición suave en width).

## Validación obligatoria por tipo

No se permite avanzar si la pregunta actual no tiene una respuesta válida. El botón **Siguiente** permanece deshabilitado y, si se intenta avanzar por teclado, se muestra el aviso inline:
**Selecciona una respuesta para continuar.**

Reglas de validación:
- **likert**: obligatorio. Si no hay `response_options`, se usa set fijo (Nunca - Casi siempre).
- **boolean**: obligatorio (Sí/No).
- **integer**: obligatorio; entero >= 0 y respeta `response_min/response_max` si existen.
- **text**: obligatorio con mínimo 3 caracteres.
...
### Text (text)
- Textarea con altura cómoda.
- Label "Respuesta" y hint de longitud mínima.
...
- En pantallas pequeñas, los controles pasan a una barra inferior sticky.

## Finalización

- En la última pregunta, el botón **Guardar** muestra un mensaje de éxito:
  - "Cuestionario guardado correctamente."
- Tras confirmar el mensaje, la vista vuelve a la bienvenida del cuestionario.
- Por ahora no se envían respuestas al backend.
